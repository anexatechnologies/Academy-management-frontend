              ...
              <div className="flex items-center gap-2 mt-1">
                {batch.courses && batch.courses.length > 0 ? (
                  <div className="flex flex-wrap gap-1">
                    {batch.courses.map((c) => (
                      <span key={c.id} className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-violet-100 text-violet-700 border border-violet-200 dark:bg-violet-900/30 dark:text-violet-400 dark:border-violet-800">
                        {c.name}
                      </span>
                    ))}
                  </div>
